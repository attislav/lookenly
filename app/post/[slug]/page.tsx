...
export async function generateMetadata({ params }: PostPageProps) {
  ...
  return {
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://lookenly.com'}/post/${slug}`,
    },
  };
}
...
export default async function PostPage({ params }: PostPageProps) {
  ...
  const category = post.categories.nodes[0]; // Primary category
  const relatedPosts = category
    ? await getRelatedPosts(category.slug, post.id, 5)
    : [];

  const currentUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://yourdomain.com'}/post/${slug}`;
